...
def import_file(network_node, file_path, file_basename, file_ext, cursor_position):
    ...
    if network_node.type().name() == "geo":
        ...
        new_node = create_new_node(network_node, file_path, "texture::2.0", "map", cursor_position, name = file_name)
        new_node.setColor(hou.Color(0.209573,0.473332,0.345818))
        return True
    elif network_node.type().name() == "redshift_vopnet":
        new_node = create_new_node(network_node, file_path, "redshift::TextureSampler", "tex0", cursor_position, name = file_name)
        new_node.setColor(hou.Color(0.209573,0.473332,0.345818))
        return True
        ...
        new_node = create_new_node(network_node, file_path, "arnold::image", "filename", cursor_position, name = file_name)
        new_node.setColor(hou.Color(0.209573,0.473332,0.345818))
        return True
